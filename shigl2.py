...
if __name__ == "__main__":
    main()
# ============================================
# ادامه کد قبلی - اضافه کردن به کلاس SHIGLInterpreter
# ============================================
...
class SHIGLInterpreter:
    ...
    def _execute_command(self, tokens: List[Token], pos: int) -> Optional[Any]:
        """اجرای یک دستور - توسعه‌یافته برای اندروید"""
        token = tokens[pos]
        cmd = token.value
    ...
    def _cmd_android(self, tokens: List[Token], pos: int) -> str:
        ...
        # ایجاد فایل‌های اصلی
        self._create_android_files(project_path, project_name)
...
🔧 مراحل بعدی:
  1. cd {project_name}
  2. شبیه‌سازی با: run emulator
  3. ساخت APK با: build apk
  4. نصب روی دستگاه: run device
        """
    
    def _create_android_files(self, project_path: str, project_name: str):
        """ایجاد فایل‌های اصلی پروژه اندروید"""
        import os
...
'''
        activity_path = os.path.join(project_path, 'app', 'src', 'main', 'java', 'com', 'example', project_name.lower(), 'MainActivity.java')
...
'''
        layout_path = os.path.join(project_path, 'app', 'src', 'main', 'res', 'layout', 'activity_main.xml')
...
'''
        build_path = os.path.join(project_path, 'app', 'build.gradle')
...
'''
    ...
    def _cmd_build(self, tokens: List[Token], pos: int) -> str:
        """ساخت APK"""
        if pos + 1 >= len(tokens):
            return "❌ فرمت: build <apk|aab> [project_path]"
        
        build_type = tokens[pos + 1].value
        
        # دریافت مسیر پروژه
        if pos + 2 < len(tokens):
            project_path = tokens[pos + 2].value
        else:
...
📱 مسیر APK: {apk_path}
📦 حجم: {os.path.getsize(apk_path) / 1024 / 1024:.2f} MB

🔧 نصب روی دستگاه:
  run device {project_path}
  
🌐 اجرا روی شبیه‌ساز:
  run emulator {project_path}
"""
                else:
...
📱 مسیر AAB: {aab_path}
📦 حجم: {os.path.getsize(aab_path) / 1024 / 1024:.2f} MB
...
"""
    ...
    def _cmd_run(self, tokens: List[Token], pos: int) -> str:
        ...
        if pos + 1 >= len(tokens):
            return "❌ فرمت: run <emulator|device> [project_path]"
        ...
        try:
            ...
            if run_target == 'emulator':
                print("📱 در حال اجرا روی شبیه‌ساز...")
                result = subprocess.run(['./gradlew', 'installDebug'], 
                                       capture_output=True, text=True, cwd=project_path)
                
                if result.returncode == 0:
                    # پیدا کردن دستگاه متصل
                    devices = subprocess.run(['adb', 'devices'], 
                                           capture_output=True, text=True)
                    
                    if 'emulator' in devices.stdout:
                        # اجرا روی شبیه‌ساز
                        subprocess.Popen(['adb', 'shell', 'am', 'start', 
                                         '-n', 'com.example.shiglapp/.MainActivity'])
                        return "✅ برنامه روی شبیه‌ساز اجرا شد!"
                    else:
                        return "⚠️  شبیه‌ساز اندروید پیدا نشد. ابتدا شبیه‌ساز را اجرا کنید."
                else:
                    return f"❌ خطا در نصب روی شبیه‌ساز:\n{result.stderr}"
            
            elif run_target == 'device':
                print("📱 در حال نصب روی دستگاه فیزیکی...")
                result = subprocess.run(['./gradlew', 'installDebug'], 
                                       capture_output=True, text=True, cwd=project_path)
                
                if result.returncode == 0:
                    return "✅ برنامه روی دستگاه نصب شد!"
                else:
                    return f"❌ خطا در نصب روی دستگاه:\n{result.stderr}"
    ...
    def _cmd_activity(self, tokens: List[Token], pos: int) -> str:
        """ایجاد Activity جدید"""
        if pos + 1 >= len(tokens):
            return "❌ فرمت: activity <name> [project_path]"
...
🔧 برای استفاده در AndroidManifest.xml:
  <activity android:name=".{activity_name}" />
    ...
    def _cmd_resource(self, tokens: List[Token], pos: int) -> str:
        """مدیریت منابع (string, color, dimension)"""
        if pos + 2 >= len(tokens):
            return "❌ فرمت: resource <string|color|dimen> <name> <value> [project_path]"
        
        resource_type = tokens[pos + 1].value
        resource_name = tokens[pos + 2].value
        resource_value = tokens[pos + 3].value if pos + 3 < len(tokens) else ''
        
        # دریافت مسیر پروژه
        if pos + 4 < len(tokens):
            project_path = tokens[pos + 4].value
        else:
            project_path = os.getcwd()
    ...
    def _cmd_manifest(self, tokens: List[Token], pos: int) -> str:
        ...
        if pos + 2 >= len(tokens):
            return "❌ فرمت: manifest <permission|feature> <value> [project_path]"
        ...
        # دریافت مسیر پروژه
        if pos + 3 < len(tokens):
            project_path = tokens[pos + 3].value
        else:
            project_path = os.getcwd()
        ...
        return f"✅ {manifest_type} {manifest_value} به AndroidManifest.xml اضافه شد"
def _cmd_help(self, tokens: List[Token], pos: int) -> str:
    """دستور help - به‌روزرسانی شده"""
    return """
📚 راهنمای SHIGL v1.0 (با پشتیبانی اندروید)
...
🚀 **دستورات اندروید (جدید!):**
  android create <name>     - ایجاد پروژه جدید اندروید
  build apk [path]          - ساخت فایل APK
  build aab [path]          - ساخت فایل AAB
  run emulator [path]       - اجرا روی شبیه‌ساز
  run device [path]         - اجرا روی دستگاه فیزیکی
  activity <name> [path]    - ایجاد Activity جدید
  resource string <name> <value> - اضافه کردن رشته
  manifest permission <name> - اضافه کردن دسترسی
